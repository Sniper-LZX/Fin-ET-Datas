...
from transformers.file_utils import cached_path, WEIGHTS_NAME, CONFIG_NAME
...
def prune_linear_layer(layer, index, dim=0):
    # layer: 要修剪的线性层
    # index：一个包含要保留的条目索引的tensor
    # dim：默认为0，指示了要在哪个维度进行索引选择
    """ Prune a linear layer (a model parameters) to keep only entries in index.
        Return the pruned layer as a new layer with requires_grad=True.
        Used to remove heads.
        修剪一个线性层(一个模型参数)，只保留索引中的条目。返回修剪后的层作为一个新层，requires_grad=True
        移除网络中不再需要的“头部”或参数
    """
    # 确保相同设备
    index = index.to(layer.weight.device)
    # 选取权重tensor中dim维度上index指定的条目，然后克隆和分离出来，作为修剪后新层的权重
...
def gelu(x):
    """
        Implementation of the gelu activation function.激活函数
        For information: OpenAI GPT's gelu is slightly different (and gives slightly different results):
        0.5 * x * (1 + torch.tanh(math.sqrt(2 / math.pi) * (x + 0.044715 * torch.pow(x, 3))))
        Also see https://arxiv.org/abs/1606.08415
    """
    # 与 OpenAI GPT 的 gelu 实现略有不同
...
# TODO 定义了一个用于存储和管理BERT模型配置的类 BertConfig
...
# TODO 一个层归一化（Layer Normalization）的类 BertLayerNorm
try:
    ...
    class BertLayerNorm(nn.Module):
        def __init__(self, hidden_size, eps=1e-12):
            ...
            # 这两个参数是通过nn.Parameter定义的，它们是层归一化中的可学习参数。
            self.weight = nn.Parameter(torch.ones(hidden_size))
            self.bias = nn.Parameter(torch.zeros(hidden_size))
            # 这是归一化过程中用于增加数值稳定性的一个小正值
            self.variance_epsilon = eps

        def forward(self, x):
            # 计算x在最后一个维度（特征维度）的平均值
            u = x.mean(-1, keepdim=True)
            # 先计算x与均值u的差，求平方，再计算在最后一个维度的平均值，得到方差
            s = (x - u).pow(2).mean(-1, keepdim=True)
            # 用x减去均值u，除以根号下的方差与self.variance_epsilon之和，得到归一化的x
            x = (x - u) / torch.sqrt(s + self.variance_epsilon)
            # 最后将归一化后的值乘以可学习的规模参数weight，再加上偏移参数bias，返回最终结果
            return self.weight * x + self.bias


# TODO 这个类负责将输入的单词ID转换成模型可以理解的嵌入向量
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # TODO 词嵌入层（从下载好的 BERT预训练模型中获得信息）
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
        # 位置嵌入层
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        # 标记类型（用于区分一个序列中的两种类型的标记）
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size)

        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        # 层归一化，有助于加快训练速度，避免过拟合
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        # 在嵌入向量上应用的dropout，用于正则化
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, input_ids, token_type_ids=None):
        seq_length = input_ids.size(1)
        # 函数计算位置ID，然后基于input_ids、生成的位置ID和token_type_ids（如果没有提供就使用全零向量），分别从三个嵌入层中获取嵌入。
        position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device)
        position_ids = position_ids.unsqueeze(0).expand_as(input_ids)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
        # TODO 嵌入（词嵌入、位置嵌入、token类型（token所属句子的索引）嵌入）
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, attention_probs=None):
        ...
        """ ------------------------------------------------------------------------ """
        # 保留多头输出
        # 这通常用于模型训练过程中，当你需要访问或者分析每个头的梯度时
        if self.keep_multihead_output:
            self.multihead_output = context_layer
            self.multihead_output.retain_grad()
        # 调整上下文层的维度
        # context_layer通过permute方法重新排列维度，然后调用contiguous方法确保张量在内存中是连续的
...
class BertCrossAttention(nn.Module):
    ...
    def forward(self, graph_vectors, word_vectors, attention_mask=None, head_mask=None, drop_first_token=True, sent_ind=None):
        ...
        # Take the dot product between "query" and "key" to get the raw attention scores.
        # TODO 使用查询层和键层的转置进行点积运算，得到原始的注意力分数
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))

        # TODO 如果提供了 sent_ind 并且配置中指定了分割句子（sep_sent），
        #  会按照指定的逻辑修改注意力分数
        ...
        # 根据多头注意力头的大小对分数进行缩放，这是Transformer模型中的一个标准操作
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
        # 如果给出了注意力掩码，则将其应用于分数上
        if attention_mask is not None:
            attention_scores = attention_scores + attention_mask
        ...
        # Mask heads if we want to
        # 如果提供了 head_mask，则用于掩蔽一部分的头部
        if head_mask is not None:
            attention_probs = attention_probs * head_mask

        # TODO 通过将注意力概率与 value 向量相乘，得到上下文向量
        context_layer = torch.matmul(attention_probs, value_layer)
        if self.keep_multihead_output:
            self.multihead_output = context_layer
            self.multihead_output.retain_grad()
        ...
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)
        # TODO 如果配置了输出注意力概率，则返回注意力概率和上下文层，否则只返回上下文层。
        if self.output_attentions:
            return attention_probs, context_layer
        return context_layer
