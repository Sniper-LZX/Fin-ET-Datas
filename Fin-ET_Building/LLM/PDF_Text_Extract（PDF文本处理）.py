...
def text_extra(chapter, add_page, begin_page, end_page, pdf, out_txt_name):
    ...
    print(chapter + '的页号为:', begin_page, '-', end_page - 1)
    begin_page += add_page
    end_page += add_page
...
def pdf2txt(pdf_file_path, odd):
    ...
    # if file_extension != '.pdf':             # 若文件不是pdf则返回
    #     print('不是pdf文件')
    #     return 0

    # 路径分割获得首个文件夹
    dir_list = str(directory).split('\\', 1)
    first_dir = dir_list[0]             # 路径中的第一个文件夹
    others = dir_list[1]                # 路径的其他成分
    ...
    print("抽取文件 ", pdf_file_path)
    '''-------------- 分析pdf，同时捕获异常 --------------'''
    try:
        pdf = pdfplumber.open(pdf_file_path)        # 使用库函数打开pdf文件
    except Exception:
        error1 = str(file_name) + " （可能是pdf文件格式的问题）"
        odd.write(error1)
        odd.write('\n')
        return -1
    else:
        out_txt_dir = os.path.join(first_dir + "Text", others)       # 正文文本文件夹路径
        '''-------------创建储存文本的文件夹-------------'''
        if not os.path.exists(out_txt_dir):
            os.makedirs(out_txt_dir)
        ''' --------------- 正文文件名 --------------- '''
        out_txt_name = os.path.join(out_txt_dir, file_name + '.txt')
        ...
        try:
            ...
            odd.write(str(pdf_file_path) + " extract_text()函数抽取异常\n")
            return -3
        ...
        try:
            for i in range(20):
                ...
                if '目录' in c_list or '目目目目录录录录' in c_list or in_punc(string):
                    ...
                    if '(cid:' in catalog:
                        odd.write(str(pdf_file_path) + ' 页面解析为(cid:xx)\n')
                        return -5
                    '''-------------------目录跨越多个页面-------------------'''
            ...
            if catalog == '':
                print("未找到目录页")
                odd.write(str(pdf_file_path) + "未找到目录页\n")
                return -4
        except Exception as e:
            odd.write(str(pdf_file_path) + " 抽取异常" + str(e) + '\n')
            return -3
        '''--------------获取目录页内容 end --------------'''
        ...
        while c < len(c_list):
            ...
            if not end2 and ("重大" in c_list[c] or "重要" in c_list[c]) and "事项" in c_list[c]:
                ...
                if begin_page2 > end_page2 or begin_page2 > page_num or end_page2 > page_num \
                    ...
                    string3 = str(file_name) + ' 重要（重大）事项' + '页号范围为:' + str(begin_page2) + '-' + str(end_page2 - 1)
                    odd.write(string3)
                    odd.write('\n')
                    return -2
                '''-------------对具体页面范围进行三元组抽取-------------'''
            ...
            if not end3 and ("变动" in c_list[c] or "股东" in c_list[c]) and "情况" in c_list[c]:
                ...
                if begin_page3 > end_page3 or begin_page3 > page_num or end_page3 > page_num \
                    ...
                    string3 = str(file_name) + ' 股本变动与股东情况' + '页号范围为:' + str(begin_page3) + '-' + str(end_page3 - 1)
                    odd.write(string3)
                    odd.write('\n')
                    return -2
                '''-------------对具体页面范围进行三元组抽取-------------'''
        ...
        if begin_page1 == 0 and begin_page2 == 0 and begin_page3 == 0 and end_page1 == 0 and end_page2 == 0 and end_page3 == 0:
            string3 = str(file_name) + " 无目录或目录页号抽取错误"
            odd.write(string3)
            odd.write('\n')
            return -4
    return 1
...
if __name__ == '__main__':
    flag = 0
    parser_pdf_count = 0        # 解析的pdf数
    cannot_parser_count = 0     # 无法解析pdf数
    total = 0                   # 文件总数

    # 所有企业年报
    files_path_list = list()
    get_file_list(folder_path, files_path_list)             # 获取文件夹里的文件，形成一个文件列表

    for file_path in files_path_list:
        error_txt = open('错误文本.txt', 'a', encoding='utf-8')  # 记录异常文件
        total += 1
        try:
            flag = pdf2txt(file_path, error_txt)
            if flag == 0:
                print("不是pdf")
            elif flag == -1:
                print("疑似pdf文件格式问题")
                cannot_parser_count += 1
            elif flag == -2:
                print("目录页号问题")
                cannot_parser_count += 1        # 目录页号问题
            elif flag == -3:
                print("抽取异常")
                cannot_parser_count += 1        # 抽取异常
            elif flag == -4:
                print("未找到目录或目录格式不对")
                cannot_parser_count += 1        # 无目录
            elif flag == -5:
                print("抽取格式错误(cid:xx)")
                cannot_parser_count += 1
            else:
                parser_pdf_count += 1
                # os.remove(item)                 # 抽取成功的文件会被删除，注意文件的保存
        except Exception as e:
            print("未知错误")
            string3 = "{}有未知错误: {} \n".format(file_path, e)
            error_txt.write(string3)
            continue
        error_txt.close()
    ...
    t = str(files_path_list[0][-27:]) + '-' + str(files_path_list[-1][-27:]) + '\n'
    t += "总共文件数量: " + str(total) + " 个\n"
    # t += "成功解析文件数量: " + str(parser_pdf_count) + " 个\n"
    # t += "解析失败文件数量: " + str(cannot_parser_count) + " 个\n"
    # t += period
    # print(period)
    print(t)
